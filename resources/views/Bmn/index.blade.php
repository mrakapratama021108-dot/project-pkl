            ...
            <div class="bg-white p-6 border border-slate-200/80 rounded-2xl shadow-sm">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-slate-200 hover:shadow-sm transition-all">
                        <div>
                            <div class="text-sm font-semibold text-slate-900">{{ $l->pegawai }}</div>
                            <div class="text-xs text-slate-500 mt-0.5">Barang: <span class="font-medium text-slate-700">{{ $l->jumlah }}x {{ $l->barang->nama_barang ?? 'Barang' }}</span></div>
                            
                            <!-- Keterangan Kondisi & Tombol Cetak BAST Pengembalian -->
                            @if($l->status === 'selesai' && $l->kondisi_kembali)
                                ...
                                <!-- Tombol Cetak BAST Pengembalian -->
                                <div class="mt-2">
                                    <a href="{{ route('bmn.pengembalian.bast', $l->id) }}" target="_blank" class="inline-flex items-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg text-xs font-semibold shadow-sm transition-all">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-slate-200 transition-all">
                        <div>
                            ...
                            @if($p->biaya_estimasi)
                                <div class="text-[11px] text-blue-700 font-medium mt-1">
                                    Est. Biaya: Rp {{ number_format($p->biaya_estimasi, 0, ',', '.') }} 
                                    • Target Selesai: {{ \Carbon\Carbon::parse($p->tanggal_estimasi_selesai)->translatedFormat('d F Y') }}
                                </div>
                            @endif
                        </div>
                        <div class="flex items-center gap-3">
                            ...
                            @if($role === 'Reviewer (Tim BMN)' && $p->status === 'menunggu_reviewer')
                                <button type="button" onclick="document.getElementById('modalPerbaikan{{ $p->id }}').showModal()" class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium">
                                    Proses Perbaikan
                                </button>
                                <form action="{{ route('bmn.perbaikan.putuskan', $p->id) }}" method="POST">
                                    @csrf @method('PATCH')
                                    <input type="hidden" name="bisa_diperbaiki" value="0">
                                    <button class="bg-rose-600 hover:bg-rose-700 text-white px-3 py-1.5 rounded-lg text-xs font-medium">Rusak Final</button>
                                </form>

                                <!-- Modal Input Estimasi -->
                                <dialog id="modalPerbaikan{{ $p->id }}" class="p-6 rounded-xl shadow-2xl backdrop:bg-slate-900/50 w-full max-w-md border border-slate-100">
                                    <div class="flex justify-between items-center mb-4">
                                        <h3 class="font-bold text-slate-800 text-sm">Input Estimasi Perbaikan</h3>
                                        <form method="dialog"><button class="text-slate-400 hover:text-slate-600 font-bold text-sm">✕</button></form>
                                    </div>
                                    <form action="{{ route('bmn.perbaikan.putuskan', $p->id) }}" method="POST" class="space-y-4">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="bisa_diperbaiki" value="1">
                                        <div>
                                            <label class="block text-xs font-semibold text-slate-600 mb-1">Estimasi Biaya Perbaikan (Rp)</label>
                                            <input type="number" name="biaya_estimasi" placeholder="Contoh: 500000" class="w-full text-xs p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required>
                                        </div>
                                        <div>
                                            <label class="block text-xs font-semibold text-slate-600 mb-1">Estimasi Selesai Perbaikan</label>
                                            <input type="date" name="tanggal_estimasi_selesai" min="{{ date('Y-m-d') }}" class="w-full text-xs p-2.5 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required>
                                        </div>
                                        <div class="flex justify-end gap-2 pt-2">
                                            <button type="button" onclick="document.getElementById('modalPerbaikan{{ $p->id }}').close()" class="px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-medium">Batal</button>
                                            <button type="submit" class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-medium">Setujui & Proses Servis</button>
                                        </div>
                                    </form>
                                </dialog>
                            @endif
        ...
        <div class="space-y-6 max-w-5xl">
            <div class="bg-white p-6 border border-slate-200/80 rounded-2xl shadow-sm">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <!-- Tombol Cetak Rekap 6 Bulanan -->
                    <a href="{{ route('bmn.laporan6bulan.cetak') }}" target="_blank" class="inline-flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-3.5 py-2 rounded-xl text-xs font-semibold shadow-sm transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                        Cetak Rekap 6 Bulanan
                    </a>
                </div>
                ...
                <div class="bg-blue-50/60 p-4 rounded-xl border border-blue-200/80 mb-6">
                    ...
                    <form action="{{ route('bmn.laporan6bulan.simpan') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        @csrf
                        <div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($laporan6Bulan as $l)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="py-3 px-3 font-semibold text-slate-800">{{ $l->pegawai }}</td>
                                <td class="py-3 px-3 text-slate-600">{{ $l->peminjaman->barang->nama_barang ?? '-' }}</td>
                                <td class="py-3 px-3 text-slate-500">{{ $l->periode }}</td>
                                <td class="py-3 px-3 text-center">
                                    <span class="bg-emerald-100 text-emerald-800 font-bold px-2.5 py-1 rounded-md text-[10px]">
                                        {{ strtoupper($l->kondisi) }} (TERVERIFIKASI)
                                    </span>
                                </td>
                            </tr>
                            @empty
